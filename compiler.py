...
class Compiler:
    def __init__(self, procedures, main_prog, tokens):
        self.div = False
        self.times = False
        self.times_is_generated = False
        self.div_is_generated = False
        self.tokens = tokens
        ...
        if self.div and not self.div_is_generated:
            self.generate_div()
            self.div_is_generated = True
        if self.times and not self.times_is_generated:
            self.genereate_times()
            self.times_is_generated = True
    ...
    def generate_div(self):
        self.memory.set_program_variable(f"DIV_START")
        self.memory.set_program_variable(f"DIV_RECALL")
        self.assembly.append(f"SET {len(self.assembly) + 3}")
        self.assembly.append(f"STORE {self.memory['DIV_START']}")
        j_index = len(self.assembly)
        self.assembly.append(f"JUMP DivFinish")
        pj = self.memory["Pj"]
        pi = self.memory["Pi"]
        pk = self.memory["Pk"]
        ...
        self.assembly.append(f"SET 1")
        self.assembly.append(f"STORE 7")
        self.assembly.append(f"JUMPI {self.memory['DIV_RECALL']}")
        self.assembly[j_index] = self.assembly[j_index].replace('DivFinish',
                                                                str(len(self.assembly)))

    def genereate_times(self):
        index_const_1 = self.memory["Pj"]
        index_helper = self.memory["Pi"]
        index_2n_val = self.memory["Pk"]
        self.memory.set_program_variable(f"TIMES_START")
        self.memory.set_program_variable(f"TIMES_RECALL")
        self.assembly.append(f"SET {len(self.assembly) + 3}")
        self.assembly.append(f"STORE {self.memory['TIMES_START']}")
        j_index = len(self.assembly)
        self.assembly.append(f"JUMP TimesFinish")
        self.assembly.append(f"SET 1")
        ...
        self.assembly.append(f"ADD 7")
        self.assembly.append(f"STORE 7")
        self.assembly.append(f"JUMPI {self.memory['TIMES_RECALL']}")
        self.assembly[j_index] = self.assembly[j_index].replace('TimesFinish',
                                                                str(len(self.assembly)))
    ...
    def check_semantic_errors(self):
        ...
        for i in range(len(self.tokens)):
            if self.tokens[i].type == 'DIV' or self.tokens[i].type == 'MOD':
                self.div = True
            if self.tokens[i].type == 'TIMES':
                self.times = True
                ...
                if self.tokens[i].type == 'BEGIN':
                        ...
                        if self.tokens[i].type == 'IDENTIFIER':
                            if self.tokens[i - 1].type == 'READ':
                                ...
                                if self.tokens[i].value not in proc_all_set_vars:
                                    raise Exception(f"Zmienna {self.tokens[i].value} nie została zainicjowana"
                                                    f" | line : {self.tokens[i].lineno}")
                            if self.tokens[i + 1].type == '(':
                                ...
                                if not proc_is_declared:
                                    raise Exception(f"Procedura {proc_name_call} nie została zainicjowana"
                                                    f" | line : {self.tokens[i].lineno}")
                                i += 2
                    ...
                    for used in proc_all_used_vars:
                        if used.value not in proc_all_set_vars:
                            raise Exception(f"Użycie nie zainicjowanej zmiennej {used.value} "
                                            f"| line : {used.lineno}")
        ...
        if proc_name == "Main":
            if expression[0] == "const":
                ...
                if expression[0] == "plus":
                    ...
                    rec_line = len(self.assembly) + 3
                    self.assembly.append(f"SET {rec_line}")
                    self.assembly.append(f"STORE {self.memory['TIMES_RECALL']}")
                    self.assembly.append(f"JUMPI {self.memory['TIMES_START']}")
                    self.assembly.append(f"LOAD 7")
                elif expression[0] == "div":
                    rec_line = len(self.assembly) + 3
                    self.assembly.append(f"SET {rec_line}")
                    self.assembly.append(f"STORE {self.memory['DIV_RECALL']}")
                    self.assembly.append(f"JUMPI {self.memory['DIV_START']}")
                    self.assembly.append(f"LOAD 7")
